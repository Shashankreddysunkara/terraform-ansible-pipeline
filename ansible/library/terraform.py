...
# Terraform Plan with variable file and Plan File and forcing init of project
- terraform:
     project_path: '{{ project_dir }}'
     plan_file: '{{ plan_file }}'
     force_init: True
...
# Terraform Destroy with variable file and Plan File
- terraform:
     project_path: '{{ project_dir }}'
     plan_file: '{{ plan_file }}'
     variables_file:
...
def preflight_validation(bin_path, project_path, variables_args=None, var_files=None, plan_file=None):

    command = [bin_path, 'validate']
...
def project_init(bin_path, project_path, variables_args=None, var_files=None, plan_file=None):
    ...
    rc, out, err = module.run_command(command, cwd=project_path)
    if rc != 0:
        module.fail_json(msg="Failed to initialize Terraform configuration files:\r\n{0}".format(err))
...
def main():
    ...
    if force_init:
        project_init(command[0], project_path,variables_args, var_files)

    preflight_validation(command[0], project_path,variables_args, var_files)
